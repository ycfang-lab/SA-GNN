...
def readanno(path):
    with open(path, 'r') as f:
        ...
        anno_dict = dict(lines)
    return anno_dict

def CSL2dict(path, anno_dict):
    train_info = {'prefix': path.rsplit('/', 2)[0]}
    test_info = {'prefix': path.rsplit('/', 2)[0]}
    train_idx = 0
    test_idx = 0
    for r, d, f in os.walk(path, followlinks = True):
        if r.endswith('color'):
            prefix, anno, vid = r.rsplit('/', 2)
            pid = vid.split('_')[0]
            if int(pid[1:]) <= 40:
                train_info[train_idx]={
                    'fileid': vid,
                    'folder': os.path.join(*r.rsplit('/', 2)[1:]),
                    'signer': pid,
                    'label': anno_dict[anno]
                }
                train_idx += 1
            else:
                test_info[test_idx]={
...
def resize_dataset_CSL(video_idx, dsize, info_dict):
    info = info_dict[video_idx]
    img_list = glob.glob(os.path.join(info_dict['prefix'], 'ccslSub',info['folder'], '*.png'))
    for img_path in img_list:
        rs_img = resize_img(img_path, dsize=dsize)
        rs_img_path = img_path.replace("ccslSub", 'ccslSub_resized')
        rs_img_dir = os.path.dirname(rs_img_path)
        if not os.path.exists(rs_img_dir):
            os.makedirs(rs_img_dir)
            cv2.imwrite(rs_img_path, rs_img)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='phoenix2014',
                        help='save prefix')
    parser.add_argument('--dataset-root', type=str, default='../dataset/phoenix2014/multisigner',
    ...
    if args.dataset == 'phoenix2014':
        ...
        # generate information dict
        anno_dict = readanno('../dataset/CSL/corpus.txt')
        train_info, test_info = CSL2dict('../dataset/CSL/ccslSub/', anno_dict)
        sign_dict = dict()
        np.save(f"./{args.dataset}/train_info.npy", train_info)
        np.save(f"./{args.dataset}/test_info.npy", test_info)
        # update the total gloss dict
        sign_dict_update(sign_dict, train_info)
        sign_dict_update(sign_dict, test_info)
        # generate groudtruth stm for evaluation
        generate_gt_stm(train_info, f"./{args.dataset}/{args.dataset}-groundtruth-train.stm")
        generate_gt_stm(test_info, f"./{args.dataset}/{args.dataset}-groundtruth-test.stm")
        # resize images
        video_index = np.arange(len(train_info) - 1)
        print(f"Resize image to {args.output_res}")
        if args.process_image:
            if args.multiprocessing:
                run_mp_cmd(10, partial(resize_dataset_CSL, dsize=args.output_res, info_dict=train_info), video_index)
            else:
                for idx in tqdm(video_index):
                    run_cmd(partial(resize_dataset_CSL, dsize=args.output_res, info_dict=train_info), idx)
                    
        video_index = np.arange(len(test_info) - 1)
        print(f"Resize image to {args.output_res}")
        if args.process_image:
            if args.multiprocessing:
                run_mp_cmd(10, partial(resize_dataset_CSL, dsize=args.output_res, info_dict=test_info), video_index)
            else:
                for idx in tqdm(video_index):
                    run_cmd(partial(resize_dataset_CSL, dsize=args.output_res, info_dict=test_info), idx)
