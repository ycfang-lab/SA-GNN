...
class Compose(object):
    ...
    def __call__(self, image, skeleton, label, file_info=None):
        for t in self.transforms:
            if file_info is not None and isinstance(t, WERAugment):
                image, skeleton, label = t(image, skeleton, label, file_info)
            else:
                image, skeleton = t(image, skeleton)
...
class ToTensor(object):
    def __call__(self, video, skeleton):
...
class RandomCrop_ske(object):
    ...
    def __call__(self, clip, skeleton):
        crop_h, crop_w = self.size
        if isinstance(clip[0], np.ndarray):
            im_h, im_w, im_c = clip[0].shape
        elif isinstance(clip[0], PIL.Image.Image):
        ...
        if crop_w > im_w:
            pad = crop_w - im_w
            skeleton[:, :, 0] = skeleton[:, :, 0] + (pad // 2)
            clip = [np.pad(img, ((0, 0), (pad // 2, pad - pad // 2), (0, 0)), 'constant', constant_values=0) for img in
                    clip]
            w1 = 0
        else:
            w1 = random.randint(0, im_w - crop_w)
            skeleton[:, :, 0] = skeleton[:, :, 0] - w1

        if crop_h > im_h:
            pad = crop_h - im_h
            skeleton[:, :, 1] = skeleton[:, :, 1] + (pad // 2)
            clip = [np.pad(img, ((pad // 2, pad - pad // 2), (0, 0), (0, 0)), 'constant', constant_values=0) for img in
                    clip]
            h1 = 0
        else:
            h1 = random.randint(0, im_h - crop_h)
            skeleton[:, :, 1] = skeleton[:, :, 1] - h1

        if isinstance(clip[0], np.ndarray):
            return [img[h1:h1 + crop_h, w1:w1 + crop_w, :] for img in clip], skeleton
        elif isinstance(clip[0], PIL.Image.Image):
            return [img.crop((w1, h1, w1 + crop_w, h1 + crop_h)) for img in clip], skeleton
...
class CenterCrop_ske(object):
    ...
    def __call__(self, clip, skeleton):
        ...
        top = int(round((im_h - new_h) / 2.))
        left = int(round((im_w - new_w) / 2.))
        skeleton[:, :, 0] = skeleton[:, :, 0] - left
        skeleton[:, :, 1] = skeleton[:, :, 1] - top
        return [img[top:top + new_h, left:left + new_w] for img in clip], skeleton
...
class RandomHorizontalFlip_ske(object):
    def __init__(self, prob):
        self.prob = prob

    def __call__(self, clip, skeleton):
        # clip B, H, W, 3
        # skeleton B, Points, axis[W, H]
        flag = random.random() < self.prob
        if flag:
            clip = np.flip(clip, axis=2)
            clip = np.ascontiguousarray(copy.deepcopy(clip))
            skeleton[:, :, 0] = clip.shape[2] - skeleton[:, :, 0] # flip
            skeleton = skeleton[:, [0, 2, 1], :] # swap two hands
        return np.array(clip), skeleton
...
class TemporalRescale(object):
    ...
    def __call__(self, clip, skeleton):
        ...
        return clip[index], skeleton[index]
...
class resize(object):
    "simply resize to (new_size, new_size)"
    def __init__(self, new_size, interp='bilinear'):
        self.interpolation = interp
    ...
    def __call__(self, clip, skeleton):
        ...
        new_size = (new_w, new_h)
        skeleton[:, :, 0] = skeleton[:, :, 0] * (new_w / width)
        skeleton[:, :, 1] = skeleton[:, :, 1] * (new_h / height)
        if isinstance(clip[0], np.ndarray):
            return [np.array(PIL.Image.fromarray(img).resize(new_size)) for img in clip], skeleton
        elif isinstance(clip[0], PIL.Image.Image):
            return [img.resize(size=(new_w, new_h), resample=self._get_PIL_interp(self.interpolation)) for img in clip], skeleton
        else:
            raise TypeError('Expected numpy.ndarray or PIL.Image' +
                            'but got list of {0}'.format(type(clip[0])))
