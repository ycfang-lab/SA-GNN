...
def seq_train(loader, model, optimizer, device, epoch_idx, recoder, CCE_activate):
    model.train()
    loss_value = []
    clr = [group['lr'] for group in optimizer.optimizer.param_groups]
    for batch_idx, data in enumerate(tqdm(loader)):
        vid = device.data_to_device(data[0])
        vlen = device.data_to_device(data[1])
        ske = device.data_to_device(data[2])

        label = device.data_to_device(data[3])
        llen = device.data_to_device(data[4])

        ret_dict = model(vid, ske, vlen)
        loss = model.criterion_calculation(ret_dict, label, llen, CCE_activate)
        if recoder.use_tb:
            recoder.add_scalar('loss/train_batch', scalar_value=loss.item(), step_type='batch')
