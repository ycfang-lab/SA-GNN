...
def evaluate(prefix="./", mode="dev", evaluate_dir=None, evaluate_prefix=None,
             ...
             triplet=False, dataset='phoenix14'):
    '''
    TODO  change file save path
    '''
    sclite_path = "./software/sclite"
    print(os.getcwd())
    if dataset == 'phoenix14':
        os.system(f"bash {evaluate_dir}/preprocess.sh {prefix + output_file} {prefix}tmp.ctm {prefix}tmp2.ctm")
    elif dataset == 'PHOENIX-2014-T':
        os.system(f"bash {evaluate_dir}/preprocess_14t.sh {prefix + output_file} {prefix}tmp2.ctm")
    elif dataset == 'CSL':
        # os.system(f"cp {prefix + output_file} {prefix}tmp2.ctm")
        os.system(f"cat {prefix + output_file} | sort -k1,1 -k3,3 > {prefix}tmp2.ctm")
    else:
        raise NameError
